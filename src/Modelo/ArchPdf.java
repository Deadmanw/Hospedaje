...
public class ArchPdf {
    ...
    public void crear_PDF(Persona cli) {
        ...
        if (this.ruta_destino != null) {
            try {
                ...
                mipdf.addTitle("Ejemplo PDF"); // se añade el titulo
                ...
                mipdf.add(new Paragraph("Datos cliente\n"));
                mipdf.add(new Paragraph(cli.toString()));
                mipdf.add(codBar(mipdf, pw, cli));
                mipdf.add(codQR(cli));
    ...
    public Image codBar(Document doc, PdfWriter pw, Persona cli) {
        PdfContentByte cimg = pw.getDirectContent(); //combierte todo a codigo de barras
        Barcode128 code128 = new Barcode128();
        code128.setCode(convertirCod(cli));
    ...
    public String convertirCod(Persona obj) {
        return obj.getId() + "-" + obj.getNom();
    }

    public Image codQR(Persona cli) {
        Image img=null;
